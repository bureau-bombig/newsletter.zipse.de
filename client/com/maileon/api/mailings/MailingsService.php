...
class com_maileon_api_mailings_MailingsService extends com_maileon_api_AbstractMaileonService
{
     ...
     * This is only possible with transaction/trigger mails and be aware to NOT add advertisements in mails you send without advertisement permission. This is meant for order confirmations and the like.
     * 
     * @param $ignorePermission can be either true or false
     */
    function setIgnorePermission($mailingId, $ignorePermission)
    {
        if ($ignorePermission === true) {
            $ignorePermission = "true";
        } else if ($ignorePermission === false) {
            $ignorePermission = "false";
        }
        return $this->post('mailings/' . $mailingId . '/settings/ignorepermission', "<ignore_permission>$ignorePermission</ignore_permission>");
    }
     ...
     */
    function isIgnorePermission($mailingId)
    {
     ...
     *  if there was a connection problem or a server error occurred
     */
    function setPreviewText($mailingId, $previewText)
    {
        return $this->post('mailings/' . $mailingId . '/contents/previewtext', "<previewtext>" . $previewText . "</previewtext>");
    }
     ...
     *  if there was a connection problem or a server error occurred
     */
    function getPreviewText($mailingId)
    {
     ...
     * For templates from the same account, relative paths can be used in the form "my template" of with sub folders "someSubFolder/my template".
     * For shared templates, an absolute path is required. The easiest way to find the correct path is to set the template
     ...
     *  if there was a connection problem or a server error occurred
     */
    function setTemplate($mailingId, $template)
    {
     ...
     * For templates from the same account, relative paths will be returned in the form "my template" of with sub folders "someSubFolder/my template".
     ...
     *  if there was a connection problem or a server error occurred
     */
    function getTemplate($mailingId)
    {
     ...
     *  if there was a connection problem or a server error occurred
     */
    function resetContentsToTemplate($mailingId)
    {
     ...
     *  if there was a connection problem or a server error occurred
     */
    function getSenderAlias($mailingId)
    {
     ...
     * @throws MaileonException
     */
    function getMailingsByStates($states, $fields = array(), $page_index = 1, $page_size = 100)
    {
        $queryParameters = array(
            'page_index' => $page_index,
            'page_size' => $page_size,
     ...
     * @throws MaileonException
     */
    function setMailingSchedule($mailingId, $date, $hours, $minutes)
    {
        $queryParameters = array(
            'date' => $date,
            'hours' => $hours,
     ...
     * @throws MaileonException
     */
    function updateMailingSchedule($mailingId, $date, $hours, $minutes)
    {
        $queryParameters = array(
            'date' => $date,
            'hours' => $hours,
     ...
     *  the result of the operation
     */
    function getReportUrl($mailingId)
    {
    ...
    function deleteAttachment($mailingId, $attachmentId)
    {
        if (empty($attachmentId)) {
            throw new com_maileon_api_MaileonAPIException("no attachment id specified");
        }
     ...
     * @throws MaileonException
     */
    function sendTestMail($mailingId, $email)
    {
        $queryParameters = array(
            'email' => $email
        );
     ...
     * @throws MaileonException
     */
    function sendTestMailToTestTargetGroup($mailingId, $testTargetGroupId)
    {
        $queryParameters = array(
